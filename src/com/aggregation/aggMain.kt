...
fun main() {
    ...
    val menuTitles = listOf("Add student", "Search for student by ID", "Search for students by course", "List of passing Students", "Quit")
    var running = true
    while (running) {
        ...
        when (input) {
            "1" -> {
                ...
                val s = Undergraduate(id, name, course, mark.toDouble())
                university.enrolStudent(s)
            }
            "2" -> {
                println("Enter ID:")
                val id = readln()
            ...
            "4" -> {
                println(university.passingStudents())
            }
